...
class BoneCopyOperator(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        if len(bpy.context.selected_objects) != 2:
            ...
            return {'FINISHED'}
        toarmature = bpy.context.view_layer.objects.active
        ...
        allobjects = bpy.context.view_layer.objects.keys()
        bpy.ops.object.select_all(action='DESELECT')
        bpy.context.scene.objects[fromarmature.name].select_set(True) 
        bpy.ops.object.duplicate()
        newarmature = None
        for i in bpy.context.view_layer.objects.keys():
            if i not in allobjects:
                newarmature = bpy.context.view_layer.objects[i]
        ...
        bpy.data.objects.remove(toarmature, do_unlink=True)
...
def edit_rig(rigname):
    ...
    bpy.context.scene.objects[rigname].select_set(True)
    bpy.context.view_layer.objects.active = bpy.context.scene.objects[rigname]
